...
namespace zen
{
    namespace data
    {
        Factory::Factory()
            : m_type_id_max(0)
            , m_type_id_num_bits(0)
        {}

        const char* Factory::get_type_name(TypeId type_id) const
        {
        ...
        size_t Factory::get_num_types() const
        {
            return m_type_registry.size();
        }

        Element* Factory::construct_element(TypeId type_id) const
        {
